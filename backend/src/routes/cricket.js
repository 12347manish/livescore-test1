...
/**
 * Cricket Routes - Real-time cricket scores
 * With OPTIMIZED caching (15s TTL)
 */

// Get all live cricket scores
router.get('/live', async (req, res) => {
  try {
    const startTime = Date.now();
    
    const result = await scoreService.getLiveCricketScores();
...
router.get('/match/:fixtureId', async (req, res) => {
  try {
    ...
    const [details, events] = await Promise.all([
      scoreService.getMatchDetails(fixtureId, 'cricket'),
